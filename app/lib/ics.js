...
export function buildCalendar(events) {
  ...
  const eventBlocks = orderedEvents
    .map((event) => {
      const description = [
        event.type ? `التصنيف: ${event.type}` : "",
        event.note ? `التفاصيل: ${event.note}` : "",
        "ضمن البرنامج الزمني الداخلي للاتحاد السعودي للسهام.",
      ]
        .filter(Boolean)
        .join("\n");
  ...
  return [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//Saudi Archery Federation//Internal Program//AR",
    "CALSCALE:GREGORIAN",
    "METHOD:PUBLISH",
    "X-WR-CALNAME:البرنامج الزمني الداخلي للاتحاد السعودي للسهام",
    "X-WR-CALDESC:البرنامج الزمني الداخلي للموسم الرياضي 2026 / 2027",
